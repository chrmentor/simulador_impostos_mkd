...
function testarCalculoLucroPresumido(dados) {
    ...
    // Adicional de IR se a base de cálculo trimestral ultrapassar o limite
    const baseCalculoTrimestral = baseCalculoIRCSLL * 3;
    let adicionalIR = 0;
    if (baseCalculoTrimestral > TABELAS.lucroPresumido.adicionalIR.limite) {
        const excedente = baseCalculoTrimestral - TABELAS.lucroPresumido.adicionalIR.limite;
        adicionalIR = (excedente / 3) * TABELAS.lucroPresumido.adicionalIR.aliquota;
        valorIR += adicionalIR;
        console.log(`Adicional de IRPJ (${(TABELAS.lucroPresumido.adicionalIR.aliquota * 100).toFixed(0)}%): R$ ${adicionalIR.toFixed(2)}`);
    } else {
        console.log(`Adicional de IRPJ: R$ 0.00 (base trimestral não excede R$ ${TABELAS.lucroPresumido.adicionalIR.limite.toFixed(2)})`);
    }
